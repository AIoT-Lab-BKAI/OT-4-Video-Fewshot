...
class CNN_BiMHM_MoLo(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if hasattr(self.args, "USE_OT") and self.args.USE_OT == True:
            dists = rearrange(frame_dists, '(tb ts) (sb ss) -> (tb sb) ts ss', tb = n_queries, sb = n_support)  # [25, 25, 8, 8]
            flows = self.ot_distance(dists)
            cum_dists = torch.sum(dists * flows, (-2, -1))
        ...
        if hasattr(self.args, "USE_OT") and self.args.USE_OT == True:
            ...
            flows = self.ot_distance(dists_motion)
            cum_dists_motion = torch.sum(dists_motion * flows, (-2, -1))
